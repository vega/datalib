...
describe('stats', function() {
  ...
  describe('covariance', function() {
    ...
    it('should throw error with inputs of unequal length', function() {
      assert.throws(function() { stats.covariance(g1, [1,2]); });
    });
  ...
  describe('linearRegression',function() {
    ...
    it('should throw error with inputs of unequal length',function() {
      assert.throws(function() { stats.linearRegression(g1,[1,2]); });
    });
  ...
  describe('stats.z.ci',function() {
    ...
    it('should accept an array and an alpha',function() {
      assert.ok(stats.z.ci(g1)[0]<stats.z.ci(g1,0.1)[0] && stats.z.ci(g1)[1]>stats.z.ci(g1,0.1)[1]);
  ...
  describe('stats.z.test',function() {
    ...
    it('should devolve gracefully if there is no variation',function() {
  ...
  describe('stats.z.pairedTest',function() {
    ...
    it('should throw an error if arrays are of unequal size', function() {
      assert.throws( function() {stats.z.pairedTest([1,2,3],[1,2]);});
    });

    it('should deal with non-valid values by ignoring them when computing the differences',function() {
      assert.equal(stats.z.pairedTest([1,2,3],[3,4,5]),stats.z.pairedTest([1,2,3,4],[3,4,5,NaN]));
    });
  });
  ...
  describe('stats.z.twoSampleTest',function() {
    ...
    it('should accept an array and accessors',function() {
      ...
      assert.equal(stats.z.twoSampleTest(table,a,b),stats.z.twoSampleTest(table,b,a));
    });
    
    it('should accept two arrays',function() {
      assert.ok(stats.z.twoSampleTest(g1,g2)<0.05);
      assert.equal(stats.z.twoSampleTest(g1,g2),stats.z.twoSampleTest(g2,g1));
    });
